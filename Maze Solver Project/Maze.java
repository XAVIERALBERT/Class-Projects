...
class Maze {
	...
	public ArrayList<Square> storePath() {
		...
		Square curSquare = this.finish;
		while ((!curSquare.equals(this.start)) && (curSquare.getPrevious() != null)) {
			if (curSquare.isVisited() == true) {
				temp.add(curSquare);
			}
			curSquare = curSquare.getPrevious();
		}
		if (curSquare.equals(this.start)) {
			temp.add(curSquare);
		}
